      ...
      <div className='p-20 mx-auto w-full overflow-y-auto'>
        ...
        <dl
          ...
          <div className='px-6 py-5 sm:p-8'>
            <dt className='text-xs font-normal text-gray-400'>{translations.N_SUCRIBED}</dt>
            <dt
              className='text-[14px] mt-2 font-semibold text-gray-900'>{paymentInfo && paymentInfo.data.payment_status.st_sub_id}</dt>
            <dd className='mt-1 flex items-baseline justify-between md:block lg:flex'>
            </dd>
          </div>
          <div className='px-6 py-5 sm:p-8'>
            <dt className='text-xs font-normal text-gray-400'>{translations.PAYMENT}</dt>
            <div className='flex items-center'>
              {paymentInfo && paymentInfo.data.payment_status.trial_end > paymentInfo.data.payment_status.trial_end ? (
                <dt
                  className='text-[14px] mt-2 font-semibold text-gray-900'>€{paymentInfo.data.payment_status.total}</dt>
              ) : (
          ...
          <div className='px-6 py-5 sm:p-8 flex items-center justify-center gap-2'>
            <Link href='https://billing.stripe.com/p/login/3cs4jw2kp1fkdMccMM' target='_blank'
                  className='text-xs font-normal text-blue-600'>{translations.MANAGE_SUBSCRIPTION}</Link>
          </div>
        </dl>
      </div>
    </LayoutCustom>
